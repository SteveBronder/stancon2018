...
for (N in N_range) {
	for (i in 1:iterations) {
		...
		# data files ----
		fn_stan_cpu <- paste0(output_dir, "/GPP/GPP_", experiment_hash_cpu, ".rds")
		fn_stan_gpu <- paste0(output_dir, "/GPP/GPP_", experiment_hash_gpu, ".rds")

		# run sampling cpu ----
		res_stan <- get_samples(
			stan_file = normalizePath("./_Simulations/_Models/gpp.stan", winslash = "/"),
			cmdstan_dir,
			normalizePath(paste0(output_dir, "/GPP/_Temp"), winslash = "/"),
			stan_data,
			experiment_hash_cpu,
			param_string
		)
		...
		# run sampling gpu ----
		res_stan <- get_samples(
			stan_file = normalizePath("./_Simulations/_Models/gpp_gpu.stan", winslash = "/"),
			cmdstan_dir,
			normalizePath(paste0(output_dir, "/GPP/_Temp"), winslash = "/"),
			stan_data,
			experiment_hash_gpu,
